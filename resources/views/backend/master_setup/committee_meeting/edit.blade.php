    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-header">
                ...
                <h5>@lang('Update Committee Meeting')</h5>
            </div>
            <div class="card-body">
                <form id="createForm" enctype="multipart/form-data">
                    @csrf
                    @if (isset($committeeMeeting))
                    @method('PUT')
                    @endif
                    <div class="row">
                                ...
                                @if (isset($newStandingCommittee) && count($newStandingCommittee) > 0)
                                    @foreach ($newStandingCommittee as $data)
                                        <option value="{{ $data->id }}" 
                                            {{( $data->id == $committeeMeeting->new_standing_committees_id )?('selected'):''}}>
                                            {{ $data->committee_name }}
                                        </option>                    
                                    @endforeach
                                @endif
                                {{-- <option value="{{ $data->id }}" {{(@$data->id)?('selected'):''}}></option> --}}
                            </select>
                            ...
                            <select name="committee_rooms_id" id="committee_rooms_id" class="committee_rooms_id @error('committee_rooms_id') is-invalid @enderror form-control form-control-sm select2">
                                @if (isset($committeeRoom) && count($committeeRoom) > 0)
                                    @foreach ($committeeRoom as $data)
                                        ...
                                        <option 
                                            value="{{ $data->id }}" 
                                            {{($data->id == $committeeMeeting->committee_rooms_id)?('selected'):''}}>
                                            {{ session()->get('language') == 'bn' ? $data->name_bn : $data->name_en }}
                                        </option>
                        ...
                        <div class="form-group col-sm-4">
                            ...
                            <select name="status" id="status" class="form-control form-control-sm">                          
                                <option value="1" {{($committeeMeeting->status == 1)?('selected'):''}}>@lang('Active')</option>
                                <option value="0" {{($committeeMeeting->status == 0)?('selected'):''}}>@lang('Cancel')</option>                                    
                            </select>
                        </div>                         
                    </div>
                    <div class="row">
                        <div class="form-group col-sm-4">
                            ...
                            {{-- <input type="text" id="datepicker" value="{{ date('d-m-y',strtotime($committeeMeeting->date_meeting)) }}"  name="date_meeting" class="date_meeting form-control form-control-sm @error('date_meeting') is-invalid @enderror" placeholder="@lang('Select Date')" autocomplete="off"> --}}
                            <input type="text" id="datepicker" value="{{ date('d-m-y',strtotime($committeeMeeting->date_meeting)) }}" class="readonly_date nano_custom_date" name="date_meeting">
                        ...
                        <div class="form-group col-sm-4">
                            ...
                            <div class="input-group">
                                <input type="time" value="{{ $committeeMeeting->time_starting }}" id="time_starting" class="time_starting form-control form-control-sm @error('time_starting') is-invalid @enderror" name="time_starting"/>
                            </div>
                            @error('time_starting')
                            <span class="text-danger">{{ $message }}</span>
                            @enderror
                        ...
                        <div class="form-group col-sm-4">
                            ...
                            <div class="input-group">
                                <input type="time" value="{{ $committeeMeeting->time_ending }}" id="time_ending" class="time_ending form-control form-control-sm @error('time_ending') is-invalid @enderror" name="time_ending"/>
                            </div>
                            @error('time_ending')
                            <span class="text-danger">{{ $message }}</span>
                            @enderror
        ...
        $(document).ready(function(){
            $('#createForm').validate({
                ...
                submitHandler: function (form) {
                    ...
                    $.ajax({
                        url : "{{route('admin.master_setup.committee_meeting.update',$committeeMeeting->id)}}",
                        data : formInfo,
                        type : "POST",
                        processData: false,
                        contentType: false,
            ...
            jQuery.validator.addClassRules({
               ...
               'time_starting' : {
                ...
                'time_ending' : {                                   
                    greaterThanTime: $('.time_ending').val()?true:false 
                }                
            });  
