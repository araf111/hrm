    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-header">
                ...
                <h5>@lang('Update Committee Room')</h5>
            </div>
            <div class="card-body">
                <form id="standingCommitteeForm" class="form-horizontal" action="{{route('admin.master_setup.committee_room.update',$committeeRoom->id)}}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @if (isset($committeeRoom))
                        @method('PUT')
                        @endif
                    <div class="row">
                        <div class="form-group col-sm-4">
                            <label for="" class="control-label">@lang('1'). @lang('Enter Committee Room Name (BN)')<span class="text-danger"> *</span></label>
                            <input id="name_bn" type="text" value="{{ $committeeRoom->name_bn }}" name="name_bn" placeholder="@lang('Enter Committee Room Name (BN)')" class="name_bn form-control @error('name_bn') is-invalid @enderror"> 
                            @error('name_bn')
                        ...
                        <div class="form-group col-sm-4">
                            ...
                            <input id="name_en" type="text" value="{{ $committeeRoom->name_en }}" name="name_en" placeholder="@lang('Enter Committee Room Name (EN)')" class="name_en form-control @error('name_en') is-invalid @enderror">
                            @error('name_en')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                    ...
                    <div class="row">
                        <div class="form-group col-sm-3">
                           ...
                           <input id="house_buildings_id" name="house_buildings_id" class="house_buildings_id form-control @error('house_buildings_id') is-invalid @enderror" type="text" value="@lang('Parliament Building')" readonly>   
                        ...
                        <div class="form-group col-sm-3">
                            ...
                            <select id="songshod_blocks_id" name="songshod_blocks_id" class="songshod_blocks_id form-control @error('songshod_blocks_id') is-invalid @enderror select2">
                                ...
                                @foreach ($songshodBlock as $list)                               
                                    <option value="{{$list->id}}" {{($committeeRoom->songshod_blocks_id==$list->id)?('selected'):''}}>
                        ...
                        <div class="form-group col-sm-3">
                            ...
                            <select id="songshod_floors_id" name="songshod_floors_id" class="songshod_floors_id form-control @error('songshod_floors_id') is-invalid @enderror select2">
                                ...
                                <option value="{{$list->id}}" {{($committeeRoom->songshod_floors_id==$list->id)?('selected'):''}}>{{(session()->get('language') =='bn')?$list->name_bn:$list->name}}</option> 
                        ...
                        <div class="form-group col-sm-3">
                            ...
                            <select id="songshod_rooms_id" name="songshod_rooms_id" class="songshod_rooms_id form-control @error('songshod_rooms_id') is-invalid @enderror select2">
                                @foreach ($songshodRoom as $list)   
                                    <option value="{{$list->id}}" {{($committeeRoom->songshod_rooms_id==$list->id)?('selected'):''}}>
                                        {{ digitDateLang($list->room) }}
                                    </option> 
                                @endforeach
                            </select>
                    ...
                    <div class="row">
                        {{-- <div class="form-group col-sm-3">
                            ...
                            <input type="text" value="{{ $committeeRoom->telephone }}" name="telephone" id="telephone" class="telephone form-control @error('telephone') is-invalid @enderror" value="" readonly>
                            @error('telephone')
                        ...
                        <div class="form-group col-sm-3">
                            ...
                            <input type="text" value="{{ $committeeRoom->pabx }}" name="pabx" id="pabx" class="pabx form-control @error('pabx') is-invalid @enderror" readonly>
                            @error('pabx')
                            <span class="text-danger">{{ $message }}</span>
                            @enderror    
                        </div> --}}
                        <div class="form-group col-sm-2">
                            ...
                            <select name="status" id="status" class="form-control form-control-sm">                          
                                <option value="1" {{ (@$committeeRoom->status == 1)?('selected'):'' }}>@lang('Active')</option>
                                <option value="0" {{ (@$committeeRoom->status == 0)?('selected'):'' }}>@lang('Inactive')</option>                                      
                            </select>                             
                        </div>
                    </div>
                    <hr>
