    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-header">
                ...
                <h5>@lang('Update Standing Committee')</h5>
            </div>
            <div class="card-body">
                <form id="createForm" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        <div class="form-group col-sm-4">
                            ...
                            <input id="committee_name" type="text" name="committee_name" placeholder="@lang('Enter Committee Name')" class='form-control form-control-sm' value="{{ $editData->committee_name }}" required>        
                        </div>
                        <div class="form-group col-sm-4">
                        ...
                        <div class="form-group col-sm-4">
                            <label class="control-label" for="ministry_id">@lang('3'). @lang('Select Ministry') <span class="text-danger"> *</span></label>
                            <select name="ministry_id" id="ministry_id" class="@error('ministry_id') is-invalid @enderror form-control form-control-sm select2">
                                ...
                                @if($data->id == $editData->ministries_id)
                    ...
                    <div class="row">
                        <div class="form-group col-sm-4">
                            ...
                            <input type="text" value="{{ $editData->date_from }}" id="datepicker" name="date_from" class="date_from form-control form-control-sm @error('date_from') is-invalid @enderror" placeholder="" autocomplete="off">
                            @error('date_from')
                        ...
                        <div class="form-group col-sm-4">
                            ...
                            <input type="text" id="datepicker2" value="{{ $editData->date_to }}"  name="date_to" class="date_to form-control form-control-sm @error('date_to') is-invalid @enderror" placeholder="" autocomplete="off">
                            @error('date_to')
                            <div class="text-danger">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="form-group col-sm-4">
                            <label class="control-label" for="committeeStatus">@lang('5'). @lang('Committee Status')</label>
                    ...
                    <div class="form-group">                        
                        <div class="row">
                            <div class="col-sm-12 col-md-12 col-lg-12">
                                <label class="control-label" for="user_id">@lang('6'). @lang('Add Standing Committee Member') </label>
                            </div>
                        </div>
                        <div id="add_extra_div">
                            @if(count(@$standingCommitteeMembers) > 0)
                            @foreach($standingCommitteeMembers as $standingCommitteeMemberKey => $standingCommitteeMember)
                            <div class="remove_extra_div">
                                <div class="col-sm-12">
                                    <div class="row">
                                        <div class="col-sm-3 form-group">
                                            <input type="hidden" name="standing_committee_member_id[{{$standingCommitteeMemberKey+1}}]" value="{{$standingCommitteeMember->id}}">
                                            <select name="user_id[{{$standingCommitteeMemberKey+1}}]" class="@error('user_id') is-invalid @enderror form-control form-control-sm select2">
                                                <option value="">@lang('Select the Name')</option>
                                                @if (isset($profiles) && count($profiles) > 0)
                                                @foreach ($profiles as $data)
                                                @if($data->user_id == $standingCommitteeMember->user_id)
                                        ...
                                        <div class="col-sm-3 form-group">
                                            <select name="designation_id[{{$standingCommitteeMemberKey+1}}]" class="@error('designation_id') is-invalid @enderror form-control form-control-sm select2">
                                                ...
                                                @if($data->id == $standingCommitteeMember->designation_id)
                                        ...
                                        <div class="col-sm-3 form-group">
                                            <select name="status[{{$standingCommitteeMemberKey+1}}]" class="form-control form-control-sm">
                                                <option {{ ($standingCommitteeMember->status == 1) ? 'selected':'' }} value="1">@lang('Active')</option>
                                                <option {{ ($standingCommitteeMember->status == 0) ? 'selected':'' }} value="0">@lang('Inactive')</option>                                     
                                            </select>
                                        </div> 
                                        <div class="col-sm-3 form-group">
                                            <i class="btn btn-info fa fa-plus-circle add_extra"></i> 
                                            @if(@$standingCommitteeMemberKey != 0)
    ...
    $(document).ready(function(){
        $('#createForm').validate({
            ...
            submitHandler: function (form) {
                ...
                $.ajax({
                    url : "{{ route('admin.master_setup.new_standing_committees.update', $editData->id) }}",
                    data : formInfo,
                    type : "POST",
                    processData: false,
                    contentType: false,
