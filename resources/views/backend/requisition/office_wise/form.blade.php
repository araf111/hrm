        ...
        <div class="card">
            ...
            <form id="ministryForm" name="ministryForm" method="POST" @if($data->id)
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-sm-4">
                            <div class="form-group">
                                ...
                                <select id="building_type" required name="building_type" class="form-control @error('building_type') is-invalid @enderror select2">
                                    @php
                    ...
                    <div class="row {{$data->building_type == 0?'displays':'hides'}}" id="hostel_portion" >
                        <div class="col-sm-4">
                            <div class="form-group">
                                <label class="control-label" for="hostel_building_id">@lang('Building Name')<span style="color: red;"> *</span></label>
                                <select id="hostel_building_id" name="hostel_building_id"
                                    class="form-control @error('hostel_building_id') is-invalid @enderror select2" required>
                                    <option value="">@lang('Selected The Building Name')</option>
                                    @foreach ($hostelBuilding as $list)
                                        @if ($list->id == $data->hostel_building_id)
                        ...
                        <div class="col-sm-4">
                            <div class="form-group">
                                <label class="control-label" for="hfloor_id">@lang('Floor No')<span style="color: red;"> *</span></label>
                                <select id="hfloor_id" name="hfloor_id" class="form-control @error('hfloor_id') is-invalid @enderror select2" required>
                                    <option value="">@lang('Select Floor Number')</option>
                                    @foreach ($hostelFloor as $list)
                                    @if($list->id==$data->hfloor_id)
                        ...
                        <div class="col-sm-4">
                            <div class="form-group">
                                ...
                                <select id="hroom_id" name="hroom_id"
                                    class="form-control @error('hroom_id') is-invalid @enderror select2" required>
                    ...
                    class="row {{$data->building_type == 1?'displays':'hides'}}">
                        ...
                        <div class="col-sm-4">
                            <div class="form-group">
                                <label for="room_id" class="control-label" for="room_id">@lang('Room')<span style="color: red;"> *</span></label>
                    ...
                    <div class="row">
                        <div class="col-sm-4">
                            <div class="form-group">
                                ...
                                <input type="number" required id="num_of_telephone" name="num_of_telephone" value="{{old('num_of_telephone', $data->num_of_telephone)}}" class="form-control @error('num_of_telephone') is-invalid @enderror" placeholder="@lang('Enter Telephone Number')" autocomplete="off" maxlength="30">

                                @error('num_of_telephone')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                                @enderror
                    ...
                    <div class="row">
                        <div class="col-sm-4">
                            <div class="form-group">
                                ...
                                <input type="number" required id="num_of_pabx" name="num_of_pabx" value="{{old('num_of_pabx', $data->num_of_pabx)}}" class="form-control @error('num_of_pabx') is-invalid @enderror" placeholder="@lang('Enter PABX Number')" autocomplete="off" maxlength="30">
                    ...
                    <div class="col-sm-12">
                        <div class="form-group text-right">
                            ...
                            <button type="button" class="btn btn-default btn-sm ion-android-arrow-back">
                                <a href="{{route('admin.requisition.office_wise_telephone_pabx.index') }}">@lang('Back')</a>
                            </button>
                        </div>
                    </div>
                </div>
...
$(document).ready(function(){
    $('#building_type').change(function() {
        ...
        if (building_type == 1) {
            ...
            $('#floor_id').attr('required', true);
            $('#room_id').attr('required', true);

            $('#hostel_building_id').removeAttr('required');
            $('#hfloor_id').removeAttr('required');
            $('#hroom_id').removeAttr('required');

            $('#hostel_building_id').prop('disabled', true);
            $('#hfloor_id').prop('disabled', true);
            ...
            $('#hostel_building_id').attr('required', true);
            $('#hfloor_id').attr('required', true);
            $('#hroom_id').attr('required', true);

            $('#block_id').removeAttr('required');
            $('#floor_id').removeAttr('required');
            ...
            $('#hostel_building_id').prop('disabled', false);
            $('#hfloor_id').prop('disabled', false);
            $('#hroom_id').prop('disabled', false);
        }
    })

    $('#hostel_building_id').change(function() {
    ...
    $('#hfloor_id').change(function() {
        let hfloor_id = $(this).val();
        $.ajax({
