<table id="dataTable" class="table table-sm table-bordered table-striped">
    ...
    @foreach($data as $list)
        ...
        <tr>
            ...
            <td>@foreach($profiles as $p)
                @if ( $list->requested_to == $p->id)
            ...
            <td class="text-center">
                @if($list->status == 0 && $list->created_by !== Auth::user()->id)
                    <a class="btn btn-sm btn-success" href="{{url('admin/profile-activities/appointments/approved')}}/{{ $list->id }}">
                        <i class="fa fa-check"></i>
                    </a>
                    <a class="btn btn-sm btn-danger" href="{{url('admin/profile-activities/appointments/declined')}}/{{ $list->id }}">
                        <i class="fa fa-times"></i>
                    </a>

                @elseif($list->status == 0 && $list->created_by == Auth::user()->id)
                    <a class="btn btn-sm btn-success" href="{{route('admin.profile-activities.appointments.edit',$list->id)}}">
                        <i class="fa fa-edit"></i>
                    </a>
                    <a class="btn btn-sm btn-danger destroy" data-route="{{route('admin.profile-activities.appointments.destroy', $list->id)}}">
