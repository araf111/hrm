...
class OrderOfDaysController extends Controller
{
    ...
    public function store(Request $request, $id = null)
    {
        ...
        $message = [
            'order_date.required'             => \Lang::get('Date is required'),
            'order_document.required'             => \Lang::get('Document is required'),
        ];

        if ($request->hasfile('order_document')) {
            $allowed_extension = ['pdf'];
            if ($file = $request->file('order_document')) {
                $extension = $file->getClientOriginalExtension();
                $file_size = $file->getSize();
                if (!in_array($extension, $allowed_extension)) {
                    $rules['order_document'] = 'required|mimes:pdf|max:5048';
                    $message['order_document.*'] = \Lang::get('pdf Files are Allowed');
                }
            }
        }
    ...
    public function currentOrderOfDay()
    {
        $start = date('Y-m-d');
        $end = date('Y-m-d');
        ...
        if(!empty($parliament_data)){
            $start = $parliament_data->date_from;
            $end = $parliament_data->date_to;
        }

        $data['order_list'] = OrderOfDay::whereBetween('order_date', [$start, $end])->get();
        if(count($data['order_list'])>0){
            foreach($data['order_list'] as $d){
                $d->doc_link = ($d->order_document!='') ? url('/public/backend/attachment') . '/' . $d->order_document : '#';
                $d->start = $d->order_date;
                //$d->title = $d->order_name;
            }
