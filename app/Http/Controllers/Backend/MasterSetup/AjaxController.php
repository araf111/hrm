...
use App\Model\BillAndLegislation;
use App\Model\BillAndLegislationTitle;
...
class AjaxController extends Controller
{
    public function clauseByParliamentBillId(Request $request)
    {
        ...
        $clauses = BillAndLegislation::where('bill_id', $parliament_bill_id)->where('parent', 0)->orderBy('id', 'asc')->get();
    ...
    public function clauseListByTolalClause(Request $request)
    {
        $bill_id = $request['bill_id'];
        
        if($bill_id != 0){
            $clauses = BillAndLegislationTitle::where('id', $bill_id)->first();
            $totla_clause = $clauses->total_clause;

            if ( $totla_clause > 0 ) {

                $output = '<option value="শিরোনাম দফা">' . 'শিরোনাম দফা' . '</option>';
                $output .= '<option value="প্রস্তাবনা দফা">' . 'প্রস্তাবনা দফা' . '</option>';
                $output .= '<option value="বলবদকরণ দফা">' . 'বলবদকরণ দফা' . '</option>';
                $output .= '<option value="তপশিল দফা ১">' . 'তপশিল দফা ১' . '</option>';
                $output .= '<option value="তপশিল দফা ২">' . 'তপশিল দফা ২' . '</option>';
                
                for ($i = 1; $i <= $totla_clause; $i++){
                    $output.= '<option value="' . $i . '">' . digitDateLang($i) . '</option>';
                }

                $data['clause_list'] = $output;

            } else {
                $data['clause_list'] = '<option value="">' . __(\Lang::get('No Data Available')) . '</option>';
            }

        }

        if (isApi()) {
            $data['totla_clause'] = $totla_clause;
    ...
    public function subClauseByParliamentBillId(Request $request)
    {
        ...
        $subClauses = BillAndLegislation::where('bill_id', $parliament_bill_id)
            ->where('parent', $parent)
            ->orderBy('id', 'asc')->get();
    ...
    public function unionListByUpazila(Request $request)
    {

        $upazila_id = $request['upazila_id'];
        
        $result = Union::where( 'upazilas.id', $upazila_id )
        ->where( 'unions.upazila_id', $upazila_id )
        ->leftJoin( 'upazilas', 'upazilas.id', '=', 'unions.upazila_id' )
        ->select( 'unions.id as id', 'unions.bn_name as bn_name', 'unions.name as name', 'upazilas.id as upazila_id', 'upazilas.bn_name as upazilaNameBng', 'upazilas.name as upazilaNameEng' )
    ...
    public function unionListByUpazilaId(Request $request)
    {
        ...
        $result = Union::whereIn( 'upazilas.id', $upazila_id )
        ->whereIn( 'unions.upazila_id', $upazila_id )
        ->leftJoin( 'upazilas', 'upazilas.id', '=', 'unions.upazila_id' )
        ->select( 'unions.id as id', 'unions.bn_name as bn_name', 'unions.name as name', 'upazilas.id as upazila_id', 'upazilas.bn_name as upazilaNameBng', 'upazilas.name as upazilaNameEng' )
        ->orderBy('upazilas.id', 'asc')
        ->get();
    ...
    public function upazilaListByDistricId(Request $request)
    {
        ...
        if (isApi()) {
            $response['status']   = 'success';
            $response['message']  = '';
            $response['api_info'] = $districtList;
            return response()->json($response);
        }
    ...
    public function districtListByDivisionId(Request $request)
    {
        ...
        if (isApi()) {
            $response['status']   = 'success';
            $response['message']  = '';
            $response['api_info'] = $divisionList;
            return response()->json($response);
        }
    ...
    public function constituenciesListByDistrictId(Request $request)
    {
        ...
        $constituenciesList = Constituency::where('parliamentNumber',$current_parliament_data->parliament_number)
        ->where('district_id', $district_id)->orderBy('name', 'asc')->get();

        if(count($constituenciesList)>0){
            foreach($constituenciesList as $c){
                if(session()->get('language')=='bn'){
                    $c->name = $c->bn_name;
                }
            }
        ...
        if (isApi()) {
            $response['status']   = 'success';
            $response['message']  = '';
            $response['api_info'] = $constituenciesList;
            return response()->json($response);
        }
    ...
    // Get session List By Parliamen Id:

    public function sessionListByParliamentId(Request $request)
    {
        $parliament_id = $request['parliament_id'];

        $sessionList = ParliamentSession::where('status', 1)->where('parliament_id', $parliament_id)->orderBy('id', 'desc')->get();
        
        if (isset($sessionList) && count($sessionList) > 0) {
            $output = '<option value="">' . __('Select Session No.') . '</option>';
            foreach ($sessionList as $key => $value) {
                $output .= '<option value="' . $value->id . '">' . \Lang::get($value->session_no) . '</option>';
            }
        } else { 
    ...
    public function circularDateBySessionId(Request $request)
    {
        $session_id = $request['session_id'];

        $lDate = date('Y-m-d');

        $dateList = Circular::where('date', '>', $lDate)->where('parliament_session_id', $session_id)->where('time', '=', '00:00:00')->orderBy('id', 'asc')->get();

        if (isset($dateList) && count($dateList) > 0) {
            $output = '<option value="">' . __('Select Date') . '</option>';
            foreach ($dateList as $key => $value) {
                $output .= '<option value="' . $value->id . '">' . digitDateLang(nanoDateFormat($value->date)) . '</option>';
            }
        } else {
            $output = '<option value="">' . __(\Lang::get('No Data Available')) . '</option>';
        }
